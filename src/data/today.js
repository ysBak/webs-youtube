import todayView01 from "../assets/img/today/todayView01.jpg";
import todayView02 from "../assets/img/today/todayView02.jpg";
import todayView03 from "../assets/img/today/todayView03.jpg";
import todayView04 from "../assets/img/today/todayView04.jpg";
import todayView05 from "../assets/img/today/todayView05.jpg";


export const todayText = [
    {
        img: todayView01,
        title: "【最短＆最安】Java Silverに合格するための勉強方法を具体的に解説！",
        desc : "Java Silver(シルバー)はOracle社による世界共通のJava資格です。Java Silver資格に最短＆最安で合格するためのオススメの勉強方法（教材や使い方、時間、目標正答率）を解説しました😄",
        author : "せかチャン - 世界一わかりやすい情報科チャンネル",
        channelId: "UC57H40XcMcL4Lbp_Uq2MVaQ",
        date: "2022/08/06",
        page: "/id"
    },{
        img: todayView02,
        title: "【JLPT直前対策！】1時間でJLPT N1文法 / 日本語能力試験 N1",
        desc : "＜JLPTの勉強にどうぞ…！＞",
        author : "日本語の森",
        channelId: "UCVx6RFaEAg46xfAsD2zz16w",
        date: "2022.11.22",
        page: "/jap"
    },{
        img: todayView03,
        title: "筋トレの頻度と筋肥大は比例するのか？プロが詳しく説明します。",
        desc : "メンバーズストアでしか観られないコンテンツ、おトクな商品などを拡充していく予定ですので、楽しみにお待ちください。",
        author : "VALX 山本義徳 筋トレ大学",
        channelId: "UC7F_CLFtxDetmUnORgmwImg",
        date: "2024.05.29",
        page: "/training"
    },{
        img: todayView04,
        title: "Breaking Down Shohei Ohtani's Swing on Japanese TV",
        desc : "The High Level Pattern goes international. To learn more, visit https://www.teachermanhitting.com/",
        author : "Teacherman Hitting",
        channelId: "UC0SrcfIvNzC5L2Tf9o9pNjg",
        date: "2023.08.02",
        page: "/baseball"
    },{
        img: todayView05,
        title: "ほろ酔い",
        desc : "Provided to YouTube by LASTRUM Music Entertainment Inc.",
        author : "あいみょん",
        channelId: "UCQG5sjqWWe4MRhYM_OAEIyw",
        date: "2018.07.09",
        page: "/music"
    },
]